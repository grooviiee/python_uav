...
class SeparatedReplayBuffer(object):
    def __init__(self, args, obs_space, share_obs_space, act_space, is_uav):
        ...
        if self.args.log_level >= 3:
            print(
                f"[REPLAYBUFFER_INIT] obs_shape dType: {type(obs_shape[-1])}, obs_shape: {obs_shape}"
            )
            print(
                f"[REPLAYBUFFER_INIT] share_obs_shape dType: {type(share_obs_shape[-1])}, share_obs_shape: {share_obs_shape}"
            )
        ...
        if is_uav == True:  # UAV
            self.share_obs = np.zeros(
                (
                    ...
                    2 + args.num_users * 2 + args.num_users,
                ),
                dtype=np.float32,
            )
            self.obs = np.zeros(
            ...
            self.share_obs = np.zeros(
                (
                    self.episode_length + 2,
                    self.n_rollout_threads,
                    args.num_mbs * 2 + args.num_users * 2 + args.num_uavs * 2,
                ),
                dtype=np.float32,
            )
            self.obs = np.zeros(
        ...
        self.value_preds = np.zeros(
            (self.episode_length + 2, self.n_rollout_threads, 1), dtype=np.float32
        )
        ...
        self.actions = np.zeros(
            (self.episode_length + 1, self.n_rollout_threads, act_shape),
            dtype=np.float32,
        )
        ...
        if self.args.log_level >= 1:
            print(
                f"[REPLAYBUFFER_INIT] share_obs Buffer size: {self.episode_length + 1}, {self.n_rollout_threads}, {res_share_obs_shape}"
            )
            print(
                f"[REPLAYBUFFER_INIT] obs Buffer size: {self.episode_length + 1}, {self.n_rollout_threads}, {res_obs_shape}"
            )
            print(
                f"[REPLAYBUFFER_INIT] actions Buffer size : {self.episode_length + 1}, {self.n_rollout_threads}, {act_shape}"
            )
        ...
        if self._use_proper_time_limits:
            ...
            if self._use_gae:  # HERE
                ...
                for step in reversed(range(self.rewards.shape[0])):
                    if self._use_popart or self._use_valuenorm:
                        print(
                            f"[COMPUTE_GAE] (Compute_returns) step: {step}, self.rewards.shape[0]: {self.rewards.shape[0]}, masks: {len(self.masks)}, rewards: {len(self.rewards)}"
                        )
                        delta = (
                            self.rewards[step]
                            + self.gamma
    ...
    def feed_forward_generator(
        ...
        print(
            f"[FEED_FORWARD_GEN] batch_size({batch_size}) = episode_length({episode_length}) * n_rollout_threads({n_rollout_threads}) / num_mini_batch({num_mini_batch})"
        )
        ...
        for indices in sampler:
            ...
            yield share_obs_batch, obs_batch, rnn_states_batch, rnn_states_critic_batch, actions_batch, value_preds_batch, return_batch, masks_batch, active_masks_batch, old_action_log_probs_batch, adv_targ, available_actions_batch, batch_size
