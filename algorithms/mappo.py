...
class MAPPOAgentTrainer:
    ...
    def train(self, is_uav, buffer, update_actor=True):
        ...
        for _ in range(self.ppo_epoch):  # epoch만큼 random sample을 뽑는다
            ...
            print(
                f"[TRAIN] data_generator ({data_generator}) num_mini_batch ({self.num_mini_batch})"
            )
            idx = 0
    ...
    def ppo_update(self, is_uav, sample_index, sample, update_actor=True):
        ...
        # Step 2. Actor update
        print(
            f"[PPO_UPDATE] action_log_probs ({action_log_probs})\nold_action_log_probs_batch ({old_action_log_probs_batch})"
        )
        imp_weights = torch.exp(
            action_log_probs - old_action_log_probs_batch[sample_index]
        )

        surr1 = imp_weights * adv_targ[sample_index]
        surr2 = (
            torch.clamp(imp_weights, 1.0 - self.clip_param, 1.0 + self.clip_param)
            * adv_targ[sample_index]
        )

        if self._use_policy_active_masks:
            policy_action_loss = (
                -torch.sum(torch.min(surr1, surr2), dim=-1, keepdim=True)
                * active_masks_batch[sample_index]
            ).sum() / active_masks_batch[sample_index].sum()
        else:
        ...
        if update_actor == False:
            print(
                f"[PPO_UPDATE] (update_actor) policy_loss: {policy_loss}, dist_entropy: {dist_entropy}, self.entropy_coef: {self.entropy_coef}"
            )
        ...
        value_loss = self.cal_value_loss(
            values,
            value_preds_batch[sample_index],
            return_batch[sample_index],
            active_masks_batch[sample_index],
        )
        ...
        print(
            f"[PPO_UPDATE] (update_critic) value_loss: {value_loss}, self.value_loss_coef: {self.value_loss_coef}"
        )
